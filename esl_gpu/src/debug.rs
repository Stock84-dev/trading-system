//! ## Plotting tips:
//! Plotting storage consists of multiple accounts and streams. Accounts are used to store multiple
//! instances of plot data, must be known at compile time (`DebugContext` takes it). A stream is a
//! vec of plot primitives (line, point, vertical bar, candle, string). It must be monotonic with
//! respect to time. ```Rust
//! CTX.debug(0, 0, timestamp_ns).add((
//!     lines!(n_positions, 0, 2),
//!     texts!((balance, s), 0, 1),
//! ));
//! ```
//! The above expression produces a stream for account 0 (first parameter) and writes it to stream 0
//! (second parameter). Every `CTX.debug()` expression must have a unique stream id. The purpose of
//! a stream is to separate out data that is generated conditionally and make it easier to plot at
//! different areas of code. A stream can contain a maximum of 8 primitives. Create another stream
//! if that's not enough. Stream ids should start at roughly 0 and there is no cap.
//! ```Rust
//!     lines!(n_positions, 0, 2),
//! ```
//! Primitive macro syntax:
//! Value or a tuple describing a single point, axis id (not implemented), sub plot id, binds to
//! specific sub plot. Color is picked based on a hash generated from an expression passed as the
...
//!
//! Known bugs:
//! By default, plot is in streaming mode. In streaming mode, char automatically zooms out to fit
//! all data. While there are small sample sizes, there might be jitter or series will disappear
//! over couple of seconds. Perform any action to exit streaming mode. Sometimes pressing and
//! holding r might stabilize series disappearing after streaming is done. Candle and bar width
//! isn't calculated correctly when view is at edges of data, zoomed in and mostly displaying
//! nothing. The whole series might disappear if it contains non finite floating point numbers.
...
impl Color {
    ...
    pub fn from_name(name: &str) -> Self {
        // Hash from AHasher isn't the same even if machine is the same. It changes based on
        // compiler version. If RUSTFLAGS are changed then different hash is generated.
        ...
        let color = Self(r | 0x40, g | 0x40, b | 0x40, 0xff);
        color
    }
...
impl DebugOptions<CandlesOptions> {
    #[inline(always)]
    pub fn up_color(mut self, up_color: Color) -> Self {
        self.options.up_fill_color = up_color;
        self.options.up_outline_color = up_color;
        self
    }

    #[inline(always)]
    pub fn down_color(mut self, down_color: Color) -> Self {
        self.options.down_fill_color = down_color;
        self.options.down_outline_color = down_color;
        self
    }
...
impl DebugOptions<TextsOptions> {
    ...
    #[inline(always)]
    pub fn align(mut self, align_x: Align, align_y: Align) -> Self {
        self.options.align = (align_x, align_y);
        self
    }
